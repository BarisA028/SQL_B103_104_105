...
--TABLODA KAC FARKLİ MEYVE VARDİR
select count(distinct urun_isim) from musteri_urun--urun cesidinin toplam sayisini verdi

select urun_isim,count(distinct urun_isim) from musteri_urun--urun isimlerini tek miktar olarak getirdi1 1 1 1 1 1 1 1
group by urun_isim;

select urun_isim,count(urun_isim) from musteri_urun--urunleri kac tane ise o kadar getirdi elma 3,armut 1
group by urun_isim;
...
alter table personel 
drop adres,drop sirket-- iki sutunu da siler
...
--TYPE/SET(modify=mysql) sutunlarin ozelliklerini degistirme
